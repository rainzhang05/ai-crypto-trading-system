...
| 1D | Deterministic runtime execution modules (`execution/*`) and replay parity | Unit + DB integration + replay determinism tests | Runtime execution/replay tests pass; mismatch count zero; coverage >= 80% |
...
- Determinism/replay parity:
  ...
  - Governance parity gates in `governance/PHASE_1D_RUNTIME_VALIDATION.sql`
- Governance SQL validations:
  - `governance/PHASE_1C_VALIDATION.sql`
  - `governance/PHASE_1D_RUNTIME_VALIDATION.sql`

## Pass/Fail Gates

- Gate A: Clean-room DB bootstrap completes with no SQL errors.
- Gate B: `governance/PHASE_1C_VALIDATION.sql` returns zero violations for all checks.
- Gate C: `governance/PHASE_1D_RUNTIME_VALIDATION.sql` returns zero violations for all checks.
...
- Gate F: Coverage across `execution/*` is >= 80% line coverage.
...
6. Enables test-only insert path on ephemeral DB via `governance/TEST_RUNTIME_INSERT_ENABLE.sql`.
