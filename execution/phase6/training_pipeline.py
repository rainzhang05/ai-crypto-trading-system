...
def _complete_training_cycle(db: Phase6Database, *, cycle_id: str, status: str, detail_tokens: Sequence[object]) -> None:
    db.execute(
        ...
        """,
        {
            "event_ts_utc": datetime.now(tz=timezone.utc),
            "event_type": "TRAINING_CYCLE",
            "status": status,
            "details": f"cycle_id={cycle_id},detail_hash={stable_hash(('training_cycle_done', cycle_id, status, *detail_tokens))}",
            "row_hash": stable_hash(("automation_event_log", "TRAINING_CYCLE", cycle_id, status, *detail_tokens)),
        },
    )
    ...
    *,
    ...
    try:
        ...
        _complete_training_cycle(
            ...
            cycle_id=cycle_id,
            status="FAILED",
            detail_tokens=(type(exc).__name__, str(exc)),
        )
        raise
