...
- All timestamps use `TIMESTAMPTZ` and are UTC.
- All critical financial values use `NUMERIC(38,18)`.
- All probability fields are constrained to `[0,1]`.
...
### 2.1 Constraint Naming Convention

- `pk_<table>`
- `fk_<child>_<parent>[_qualifier]`
- `uq_<table>_<key>`
- `ck_<table>_<rule>`

### 2.2 Core Reference Tables
...
### 2.5 Feature and Model Output Tables
...
### 2.6 Portfolio, Risk, and Signal Tables
...
### 2.7 Execution and Accounting Tables
...
- No retention policy for:
  ...
  - `portfolio_hourly_state`
  - `risk_hourly_state`
- Optional retention on `order_book_snapshot` only after verified cold archive.
- Hypertable partitioning is the primary partitioning strategy. No additional native PostgreSQL partitioning required at initial production cut.

---

## 5. CARDINALITY & PERFORMANCE NOTES

| Heavy-write table | Expected growth/day | Index cost consideration | Write amplification risk | `NUMERIC(38,18)` storage implication |
|---|---:|---|---|---|
| `market_ohlcv_hourly` | `24 * A * V` | Moderate | Low | High row width across OHLCV fields |
| `order_book_snapshot` | `24 * A * S` | High if dual time indexes retained | Medium | Very high due bid/ask/size precision |
| `feature_snapshot` | `24 * A * F` | High (`asset,time`, `feature,time`, `mode,time`) | High | High due wide cardinality + numeric value per feature |
| `model_prediction` | `24 * A * H * M` | High plus partial unique META index | High | Moderate-high due expected return precision |
| `meta_learner_component` | `24 * A * H * B` | Moderate | Medium | Moderate |
| `order_request` | `O` | Moderate | Medium | Moderate |
| `order_fill` | `T` | Moderate-high | Medium-high | High due price/qty/notional/fees |
| `cash_ledger` | `~2T to 4T` | Moderate | Medium-high | High due high-frequency balance snapshots |
| `position_hourly_state` | `24 * P_active` | Low-moderate | Low | High numeric density |
| `portfolio_hourly_state` | `24 * N_accounts` | Low | Low | Moderate |
| `risk_hourly_state` | `24 * N_accounts` | Low | Low | Moderate |

Symbols:
- `A` = active assets
- `V` = venues
- `S` = order book snapshots per hour per asset
- `F` = features per asset/hour
- `H` = horizons (`3`)
- `M` = model outputs per horizon
- `B` = base models feeding meta-learner
- `O` = order attempts/day
- `T` = fills/day
...
- No hidden leverage is enforced by:
  - `ck_order_request_no_leverage_buy`
  - `ck_cash_ledger_balance_nonneg`
  - `ck_portfolio_hourly_state_cash_nonneg`
- No nullable critical financial execution fields in:
  - `order_fill`
  - `executed_trade`
  - `position_lot`
  - `cash_ledger`
  - `portfolio_hourly_state`
  - `risk_hourly_state`
- Duplicate fill inflation is prevented by:
  - `uq_order_fill_order_exchange_trade`
  - append-only trigger on `order_fill`
- Backtest/live isolation integrity is enforced by:
  - `run_mode` across trading-layer tables
  - composite FK links to `run_context`
- Deterministic replay guarantee is enforced by:
