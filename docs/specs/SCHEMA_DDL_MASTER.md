...
Version: 1.3
Status: LOCKED (PHASE 0-3 CANONICAL SNAPSHOT)
Last Synced From: `/schema_bootstrap.sql` on 2026-03-01 UTC

Canonical executable schema authority:
- `/schema_bootstrap.sql`

This document is intentionally synchronized to the full canonical SQL snapshot.
For any schema dispute, `/schema_bootstrap.sql` is authoritative.

## 0. Global Invariants

- All timestamps use `TIMESTAMPTZ` and are UTC.
- All critical financial values use `NUMERIC(38,18)`.
- Decision-path financial tables are append-only via triggers.
- Deterministic replay lineage keys and hashes are preserved as first-class constraints.
- Long-only / no-leverage safety enforcement is encoded at schema level.

## 1. Canonical SQL Snapshot (Table-By-Table + Constraints + Triggers)
